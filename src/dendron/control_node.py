...
class ParallelNode(ControlNode):
    ...
    def tick(self) -> NodeStatus:
        ...
        # Count success and failure children
        success_count = sum(1 for status in self.completed_children.values() if status == NodeStatus.SUCCESS)
        failure_count = sum(1 for status in self.completed_children.values() if status == NodeStatus.FAILURE)
        completed_count = len(self.completed_children)
